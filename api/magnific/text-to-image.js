/**
 * Vercel Serverless Function: Magnific Text to Image Gateway
 * Route: /api/magnific/text-to-image
 */
...
export default async function handler(req, res) {
  ...
  const model = req.query?.model || new URL(req.url, `http://${req.headers.host}`).searchParams.get('model') || 'flux-dev';
  const targetUrl = `https://api.magnific.com/v1/ai/text-to-image/${model}`;
  ...
  try {
    ...
    console.error('[Magnific text-to-image Error]', error);
